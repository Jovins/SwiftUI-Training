...
final class ImageStore {
    ...
    func image(name: String, size: Int) -> Image {
        ...
        let index = guaranteeInitialImage(name: name)
        let sizedImage = images.values[index][size] ?? sizeImage(images.values[index][ImageStore.originalSize]!, to: size * ImageStore.scale)
        images.values[index][size] = sizedImage
        return Image(sizedImage, scale: Length(ImageStore.scale), label: Text(verbatim: name))
    }
    
    fileprivate func guaranteeInitialImage(name: String) -> _ImageDictionary.Index {
    ...
    fileprivate func sizeImage(_ image: CGImage, to size: Int) -> CGImage {
